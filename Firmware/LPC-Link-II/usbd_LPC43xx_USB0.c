 ...
 *      RL-ARM - USB
 *----------------------------------------------------------------------------
 *      Name:    usbd_LPC43xx_USB0.c
 *      Purpose: Hardware Layer module for NXP LPC43xx
 *      Rev.:    V4.50
 *----------------------------------------------------------------------------
...
#define ENDPTCTRL(EPNum)  *(volatile uint32_t *)((uint32_t)(&LPC_USB0->ENDPTCTRL0) + 4 * EPNum)
...
uint8_t __align(4096) EPBufPool[              
                                ...
                                USBD_CDC_ENABLE * ((HS(USBD_CDC_HS_ENABLE) ? USBD_CDC_HS_WMAXPACKETSIZE  : USBD_CDC_WMAXPACKETSIZE)      +
                                                   (HS(USBD_CDC_HS_ENABLE) ? USBD_CDC_HS_WMAXPACKETSIZE1 : USBD_CDC_WMAXPACKETSIZE1) * 2 )];
#endif
...
void USBD_Init (void) {
  ...
  LPC_USB0->USBCMD_D |= (1UL << 1);     /* usb reset                          */
  while (LPC_USB0->USBCMD_D & (1UL << 1));

  LPC_CREG->CREG0 &= ~(1 << 5);

  LPC_USB0->USBMODE_D  = 2 | (1UL << 3);/* device mode                        */

#if USBD_HS_ENABLE
  LPC_USB0->PORTSC1_D &= ~(1UL << 24);
#else
  LPC_USB0->PORTSC1_D |=  (1UL << 24);
#endif


  LPC_USB0->OTGSC = 1 | (1UL << 3);

  Ep[EP_OUT_IDX(0)].maxPacket = USBD_MAX_PACKET0;

  LPC_USB0->USBINTR_D  = (1UL << 0 ) |  /* usb int enable                     */
...
void USBD_Connect (uint32_t con) {
  if (con) {
    LPC_USB0->USBCMD_D |= 1;            /* run                                */
  }
  else {
    LPC_USB0->USBCMD_D &= ~1;           /* stop                               */
  }
}
...
void USBD_Reset (void) {
  ...
  LPC_USB0->ENDPOINTLISTADDR = (uint32_t)EPQHx;

  LPC_USB0->USBMODE_D |= (1UL << 3 );   /* Setup lockouts off                 */
  LPC_USB0->ENDPTCTRL0 = 0x00C000C0;
}
...
void USBD_SetAddress (uint32_t adr, uint32_t setup) {
  if (setup == 0) {
    LPC_USB0->DEVICEADDR  = (adr << 25);
    LPC_USB0->DEVICEADDR |= (1UL << 24);
  }
}
...
void USBD_PrimeEp (uint32_t EPNum, uint32_t cnt) {
  ...
  LPC_USB0->ENDPTPRIME = (val);
  while ((LPC_USB0->ENDPTPRIME & (1 << val)));
}
...
uint32_t USBD_WriteEP (uint32_t EPNum, uint8_t *pData, uint32_t cnt) {
  ...
  LPC_USB0->ENDPTCOMPLETE = (1UL << (EPNum + 16));
...
uint32_t USBD_GetFrame (void) {
  return ((LPC_USB0->FRINDEX_D >> 3) & 0x0FFF);
}
...
void USB0_IRQHandler (void) {
  uint32_t sts, cmpl,nak, num;

  sts  = LPC_USB0->USBSTS_D & LPC_USB0->USBINTR_D;
  cmpl = LPC_USB0->ENDPTCOMPLETE;
  nak  = LPC_USB0->ENDPTNAK & LPC_USB0->ENDPTNAKEN;

  LPC_USB0->USBSTS_D      =  sts;               /* clear interupt flags       */
  ...
  if (sts & (1UL << 2)) {
    if (((LPC_USB0->PORTSC1_D >> 26) & 0x03) == 2) {
